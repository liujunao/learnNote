...
- **对垃圾回收停顿的改善**： 堆外内存直接受操作系统管理(而不是虚拟机)，这样能保持一个较小的堆内内存，以减少垃圾收集对应用的影响
...
- 在 Eden 区出生且经历过 Minor GC 后存活的对象，当能被 Servivor 容纳时，将被移动到 Servivor 空间中，并且把对象年龄设置成为 1
- 对象在 Servivor 区中每熬过一次 Minor GC，年龄就增加1岁，当年龄增加到一定程度(默认15岁)，就将会被晋级到老年代中
