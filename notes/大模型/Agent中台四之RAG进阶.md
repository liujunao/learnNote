...
- **暴力搜索**：在「全空间」进行搜索
- **ANN(近邻相似检索)方法**：通过「对全空间分割(聚类)」，将其分割成很多小的子空间
- **PQ乘积量化计算**：除了聚类外，将向量切分为多段
- **`IVFPQ`(倒排PQ乘积量化)**：每来一个query向量，首先计算其与每个粗聚类簇心的距离，然后选择距离最近的top N个簇，只计算查询向量与这几个簇底下的向量的距离
...
https://www.cnblogs.com/bubblebeee/p/16170989.html
...
> `IVF` 即 `Inverted File System`(倒排文件系统)

- **聚类算法的问题**：内存占用太大，体现在两个方面
    - 首先需要保存每个向量的坐标，而每个坐标都是一个浮点数，占用内存非常大
    - 还需要维护聚类中心和每个向量的聚类中心索引，也会占用大量的内存

- `PQ`(乘积量化)：将向量分解为多个子向量，然后对每个子向量独立进行量化
- `IVFPQ`：倒排 PQ

### 3.4 HNSW(Hierarchical Navigable Small Worlds)
...
- **Random Projection for LSH 随机投影**：将高维空间的数据点投影到低维空间，从而减少计算的时间和提高查询的质量

    > 使用随机投影矩阵将高维向量投影到低维空间中。创建一个由随机数构成的矩阵，其大小将是所需的目标低维值。然后，计算输入向量和矩阵之间的点积，得到一个被投影的矩阵，它比原始向量具有更少的维度但仍保留了它们之间的相似性
