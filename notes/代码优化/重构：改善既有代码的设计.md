...
- 若直接访问一个字段，但与字段之间的耦合关系逐渐变得笨拙，则为该字段建立取值/设值函数，并且只以这些函数来访问字段
...
- 若有一个数据项，需要与其他数据和行为一起使用才有意义，则将数据项变成对象
...
- 若从一个类衍生出许多彼此相等的实例，希望将它们替换为同一个对象，则将这个值对象变成引用对象
...
- 若有一个引用对象，很小且不可变，而且不易管理，则将它变成一个值对象
...
- 若有一个数组，其中的元素各自代码不同的东西，则以对象替换数组，对于数组中的每个元素，以一个字段来表示
...
- 若有些领域数据置身于 GUI 控件中，而领域含糊需要访问这些数据，则将该数据复制到一个领域对象中，建立一个 Observer 模式，用以同步领域对象和 GUI 对象内的重复数据
...
- 若两个类都需要使用对方特性，但其间只有一条单向连接，则添加一个反向指针，并使修改函数能同时更新两条连接
...
- 若有一个字面数值带有特别含义，则创造一个常量，根据其意义为它命名，并将上述的字面数值替换为这个常量
...
- 若类中存在一个 public 字段，则声明为 private，并提供相应的访问函数
...
- 若有个函数返回一个集合，则让这个函数返回该集合的一个只读副本，并在这个类中提供添加/移除集合元素的函数
...
- 若需要面对传统编程环境中的记录结构，则为该记录创建一个”哑“数据对象
...
- 若类中有一个数值类型码，但它不影响类的行为，则以一个新的类替换该数值类型码
...
- 若有一个不可变的类型码，它会影响类的行为，则以子类取代这个类型码
...
- 若有一个类型码，其会影响类的行为，但无法通过继承手段消除，则以状态对象取代类型码
...
- 若各个子类的唯一差别只在”返回常量数据“的函数身上，则修改这些函数，使它们返回超类中的某个(新增)字段，然后销毁子类
...
- 若有一个复杂的条件(if-then-else)语句，则从 if, then, else 三个段落中分别提炼出独立函数
...
- 若有一系列测试，都得到相同结果，则将这些测试合并为一个条件表达式，并将这个条件表达式提炼成为一个独立函数
...
- 若在一系列布尔表达式中，某个变量带有”控制标记“的作用，则以 break 语句或 return 语句取代控制标记
...
- 若函数中的条件逻辑使人难以看清正常的执行路径，则使用卫语句表现所有特殊情况
...
- 若有个条件表达式根据对象类型的不同而选择不同的行为，则将该条件表达式的每个分支放进一个子类内的覆写函数中，然后将原始函数声明为抽象函数
...
- 若需要再三检查某对象是否为 null，则将 null 值替换为 null 对象
...
- 若某段代码需要对程序状态做出某种假设，则断言明确表现这种假设
...
- 若某个函数需要从调用端得到更多信息，则为此函数添加一个对象参数，让该对象带进函数所需信息
...
- 若某个函数既返回对象状态值，又修改对象状态，则建立两个不同的函数，其中一个负责查询，另一个负责修改
...
- 若若干工作做了类似的工作，但在函数本体中却包含了不同的值，则建立单一函数，以参数表达那些不同的值
...
- 若有一个函数，其中完全取决于参数值而采取不同行为，则针对该参数的每个可能值，建立一个独立函数
...
- 若从某个对象中取出若干值，将它们作为某次函数调用时的参数，则改为传递整个对象
...
- 若对象调用某个函数，并将所得结果作为参数，传递给另一个函数，而接受该参数的函数本身也能调用前一个函数，则让参数接受者去除该项参数，并直接调用前一个函数
...
- 若类中的某个字段应该在对象创建时被设值，然后就不再改变，则去掉该字段的所有设值函数
...
- 若有一个函数，从来没有被其他任何类用到，则将这个函数值改为 private
...
- 若希望在创建对象时不仅仅做简单的构建工作，则将构造函数替换为工厂函数
...
- 若某个函数返回的对象，需要由函数调用者执行向下转型，则将向下转型动作移到函数中
...
- 若某个函数返回一个特定的代码，用以表示某种错误情况，则改用异常
...
- 若面对一个调用者可以预先检查的条件，抛出了一个异常，则修改调用者，使其在调用函数前先做检查
...
- 若有些函数，在各个子类中产生完全相同的结果，则将该函数移至超类
...
- 若在各个子类中拥有一些构造函数，它们的本体几乎完全一致，则在超类中新建一个构造函数，并在子类构造函数中调用它
...
- 若超类中的某个函数只与部分子类有关，则将该函数移到相关子类中
...
- 若超类中的某个字段只被部分子类用到，则将该字段移到需要它的子类中
...
- 若类中的某些特性只被某些实例用到，则新建一个子类，将上面所说的那一部分特性移到子类中
...
- 若两个类有相似特性，则为这两个类建立一个超类，将相同特性移至超类
...
- 若若干客户使用类接口中的同一子集，或两个类的接口部分相同，则将相同的子集提炼到一个独立接口中
...
- 若有一些子类，其中相应的某些函数以相同顺序执行类似的操作，但各个操作的细节上有所不同，则将这些操作分别放进独立函数中，并保持它们都有相同的签名，于是原函数也就变得相同了，然后将原函数上移至超类
...
- 若某个子类只使用超类接口中的一部分，或是根本不需要继承而来的数据，则在子类中新建一个字段用以保存超类，调整子类函数，令它改而委托超类，然后去掉两者之间的继承关系
...
- 若在两个类间使用委托关系，并经常为整个接口编写许多极简单的委托函数，则让委托类继承受托类
...
- 若某个继承体系同时承担两项责任，则建立两个继承体系，并通过委托关系让其中一个可以调用另一个
...
- 若有一些传统过程化风格的代码，则将数据记录变成对象，将大块的行为分成小块，并将行为移入相关对象中
...
- 若某些 GUI 类中包含了领域逻辑，则将领域逻辑分离出来，为它们建立独立的领域类
...
- 若有某个类做了太多工作，其中一部分工作是以大量条件表达式完成，则建立继承体系，以一个子类表示一种特殊情况
