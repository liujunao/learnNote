...
| 存储引擎          | 锁粒度                 | 对比                                                   |
| :---------------- | :--------------------- | ------------------------------------------------------ |
| MyISAM            | 表锁                   | 并发读没问题，并发写性能差                             |
| SQL Server 2005前 | 页锁                   | 比表锁性能有所提升，对于热点数据页并发无能为力         |
| SQL Server 2005   | 乐观并发：行锁有锁升级 | 锁越多开销越大有锁升级，升级到表锁性能回到从前         |
| InnoDB            | 行锁                   | 一致性非锁定读行锁没有相关开销，同时得到并发性和一致性 |
...
- `lock` ： 对象是事务，用来锁定数据库中的对象，如表、页、行；且一般 lock 的对象仅在事务 commit或 rollback 后进行释放(不同事务隔离级别释放的时间可能不同)，有死锁检测机制			
...
- **共享锁(S Lock)**：事务拿到某一行记录共享 S 锁，才能读取，提高读读并发

    ```sql
    SELECT * FROM table_name WHERE ... LOCK IN SHARE MODE
    ```

- **排他锁(X Lock)**：事务拿到某一行记录排他 X 锁，修改或删除，用来保证数据强一致
...
<img src="../../../pics/mysql/global/mysqlG6_6.png" align=left width=800>

#### 3. 意向锁和行级锁的兼容性

<img src="../../../pics/mysql/global/mysqlG6_7.png" align=left width=900>

### (2) 一致性锁

#### 1. 一致性的非锁定读

> 指 InnoDB 存储引擎通过多版本控制的方式来读取当前执行时间数据库中行的数据

注：若读取的行正在执行 DELETE 或 UPDATE 操作，InnoDB 存储引擎会去读取行的一个**快照数据** 

<img src="../../../pics/mysql/global/mysqlG6_3.png" align=left width=500>

---

- **非锁定读**： 不需要等待访问的行上 X 锁的释放

    > InnoDB 的默认读取方式，即读取不会占用和等待表上的锁

- **快照数据**： 指该行的之前版本的数据，该实现是通过 undo 段来完成

    > undo 用来在事务中回滚数据，因此快照数据本身是没有额外的开销，且读取快照数据不需要上锁

- **多版本并发控制(MVCC)**： 每行记录有多个快照，并由此而带来的并发控制

    > - `READ COMMITTED`：对于快照数据，非一致性读总是读取被锁定行的最新一份快照数据
...
<img src="../../../pics/mysql/global/mysqlG6_8.png" align=left width=800>

- **时间 5 结果**：`READ COMMITTED` 与 `REPEATABLE READ` 结果均为 `1`
- **时间 7 结果**：`READ COMMITTED` 结果为 `3`，`REPEATABLE READ` 结果为 `1` 

#### 2. 一致性锁定读

- 使用场景：需要显示地对数据库读取操作进行加锁以保证数据逻辑的一致性

    > 下述操作必须在一个事务中，当事务提交，锁也释放，因此需加上 `BEGIN, START TRANSACTION 或 SET AUTOCOMMIT = 0`

    - `SELECT ... FOR UPDATE`： 对读取的行记录加一个 X 锁，其他事务不能对已锁定的行加上任何锁
    - `SELECT ... LOCK IN SHARE MODE`： 对读取的行记录加上一个 S 锁，其他事务可以向被锁定行加 S 锁

> 注：对于一致性非锁定读，即使读取的行已被执行 `SELECT ... FOR UPDATE`，也可以进行读取

### (3) 自增长与锁

- InnoDB 对每个含有自增长值的表都有一个自增长计数器，当对含有自增长的计数器的表进行插入操作时，这个计数器会被初始化

    > 得到计数器值： `SELECT MAX(auto_inc_col) FROM t FOR UPDATE;`

- `AUTO-INC Locking`： 插入操作会依据自增长的计数器值加 `1` 赋予自增长列

  > - **优点**：为提高插入性能，锁不是在一个事务完成后释放，而是在完成对自增长值插入的 SQL 语句后立即释放
  > - **不足**： 
...
- 外键列若没有显式加索引，InnoDB 会自动对其加一个索引，因为这样可以避免表锁

- 插入或更新外键值时，会使用 `SELECT ... LOCK IN SHARE MODE` 读取父表，即对父表加 S 锁

    > 若这时父表已经加 X 锁，则子表上的操作会被阻塞
...
> `Phantom Problem`： 指在同一事务下，连续执行两次同样的 SQL 语句导致不同结果，第二次SQL语句可能返回之前不存在的行

- InnoDB存储引擎**采用 Next-Key Locking 机制来避免幻读** 

<img src="../../../pics/mysql/global/mysqlG6_9.png" align=left width=800>

## 5、锁问题

### (1) 脏读

- **脏页**： 指在缓冲池中已被修改的页，但还没有刷新到磁盘中，即数据库实例内存中的页和磁盘中的页的数据不一致，当然在刷新到磁盘之前，日志都已经被写入到了重做日志文件中
...
- **脏数据**： 是指事务对缓冲池中行记录的修改，并且还没有被提交，即指未提交的数据
  ...
  > 若读到了脏数据，即一个事务可以读到另外一个事务中未提交的数据，则显然违反了数据库的隔离性

- **脏读**： 指的就是在不同的事务下，当前事务可以读到另外事务未提交的数据，即读到脏数据

<img src="../../../pics/mysql/global/mysqlG6_10.png" align=left width=900>
...
- **不可重复读和脏读的区别**： 脏读是读到未提交的数据，而不可重复读则读到的是已提交数据，但违反了数据库事务一致性的要求

<img src="../../../pics/mysql/global/mysqlG6_11.png" align=left width=900>
...
- 下面的情况会发生丢失更新：
  ...
  > 避免丢失更新：让事务在上面情况下的操作变成串行化，而不是并行操作

<img src="../../../pics/mysql/global/mysqlG6_12.png" align=left width=900>

## 6、阻塞

- 参数 `innodb_lock_wait_timeout` ： 用来控制等待的时间，默认为 50s

  > 调整：`SET @@innodb_lock_wait_timeout = 60`
...
- **解决死锁问题的两种方式**：
  ...
  - **超时**：即两个事务互相等待时，当一个等待时间超过阈值，其中一个事务进行回滚，另一个事务就能继续进行

    > 参数 `innodb_lock_wait_timeout`  用来设置超时时间
    >
    > - 超时根据 FIFO 的顺序选择回滚对象，若超时事务所占权重较大，则 FIFO 方式不太合适

  - **`wait-for graph`(等待图)**： **进行死锁检测**，一种更为主动的死锁检测方式(InnoDB 存储引擎也采用该方式)
    ...
    > - 事务 T1 最终等待 T2 所占用的资源，即事务之间在等待相同的资源，而事务 T1 发生在事务 T2 的后面
...
> 指将当前锁的粒度降低，如：将行锁升级为页锁，或将页锁升级为表锁

- InnoDB 不存在锁升级问题，因为其不是根据每个记录来产生行锁，而是根据每个事务访问的每个页对锁进行管理，采用位图方式

    > 因此一个事务锁住页中一个或多个记录，其开销通常一致
