...
- **隐式(类型/字符编码)转换**：
    - 隐式类型转换对于 JOOQ 框架来说一般不会出现
    - 隐式字符编码转换在连表查询时可能出现，即连表字段的类型相同但字符编码不同

- **破坏了有序性**：比如：`like` 语句开头为 `%` 、字符串未加 `''` 等
- **区分度低的字段不要加索引**：如性别字段(0--男，1--女)

### (3) 好用且简单的索引方法

- **索引下推**：性别字段不适合建索引，但若是多条件查询，可以建立联合索引利用该特性优化

- **覆盖索引**：对于联合索引，索引中已经包含查询需要的信息，就不会再回表

- **前缀索引**：对于字符串，可以只在前 N 位添加索引，避免不必要的开支

    > 对于关键字查询，交给 ES 或许更好

- 不要对索引字段做函数操作

- 对于确定的、写多读少的表或频繁更新的字段都应该考虑索引的维护成本

## 2、等 MDL 锁

> 读写锁、写锁间互斥：
>
> - 对一个表做 CRUD 操作时，自动加 MDL 读锁
> - 对表结构做变更时，加 MDL 写锁
>
> 当某语句拿 MDL 写锁就会阻塞 MDL 读锁，`show processlist` 命令查看处于 Waiting for table metadata lock 状态的语句
...
flush 很快，大多是因为 flush 命令被别的语句堵住，它又堵住了 select 。

`show processlist` 命令查看发现处于 Waiting for table flush 状态
...
设想一个场景：事物 A 开始事务，事务 B 也开始执行大量更新。B 率先提交， A 是当前读，就要依次执行 undo log ，直到找到事务 B 开始前的值



## 6、大表场景

基础知识：对于大表常见优化是分库分表和读写分离

- `Innodb` 每个 B+ 树节点存储容量是 `16KB`，理论可存储 `2kw` 行左右，这时树高为 3 层

- `innodb_buffer_pool` 用来缓存表及索引，若索引数据较大，缓存命中率堪忧

    同时 `innodb_buffer_pool` 采用 LRU 算法进行页面淘汰，若数据量过大，对老或非热点数据的查询可能会把热点数据挤出去
...
- 分库或分表的具体分析：
    - 若磁盘或网络有 IO 瓶颈，则分库和垂直分表
    - 若是 CPU 瓶颈，即查询效率偏低，则水平分表
...
> 先评估分库、表数，制定分片规则选 key，再开发和数据迁移，还要考虑扩容问题

#### 2. 问题

主要问题：唯一 ID 生成、非 partition key 查询、扩容

- 唯一 ID 方法：DB 自增、Snowflake、号段、GUID 算法等

- 非 partition key 查询：常用映射法解决，映射表用到覆盖索引还是很快

- 扩容：根据分片时的策略确定，范围分片很简单，而随机取模分片要迁移数据

    > 可以用范围 + 取模的模式分片，先取模再范围，可以避免一定程度的数据迁移

### (2) 读写分离

- 读写分离的原因：
    - 分表针对大表解决 CPU 瓶颈，分库解决 IO 瓶颈，二者解决存储压力，但未解决查询压力
    - 若读远大于写，可以考虑读写分离，基于主从模式将读的压力分摊，避免单机负载过高，同时也保证高可用，实现负载均衡

- 问题：过期读、分配机制
    - 过期读：即主从延时问题
    - 分配机制：是走主还是从库，可以直接在代码中根据语句类型切换或使用中间件
