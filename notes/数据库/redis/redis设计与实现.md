...
- **迁移**：工具 redis-trib 可以让运维人员手动调整槽位的分配情况
  ...
  > 
  >
  > ![](/Users/yinren/allText/learnNote/pics/redis/redis_22.png)
  >
...
- **增量同步**：**Redis 同步的是指令流**
  ...
  >
  > ![](/Users/yinren/allText/learnNote/pics/redis/redis_20.png)
  >
...
- **快照同步**：快照同步很耗费资源
  ...
  **不足**：
  ...
  ![](/Users/yinren/allText/learnNote/pics/redis/redis_21.png)
...
![](/Users/yinren/allText/learnNote/pics/redis/redis_7.png)
...
![](/Users/yinren/allText/learnNote/pics/redis/redis_8.png)
...
- **非阻塞 IO**：在套接字对象上提供了一个选项 `Non_Blocking`
  ...
  >
  > ![](/Users/yinren/allText/learnNote/pics/redis/redis_9.png)
...
- **快照原理(RDB)**：Redis 使用操作系统的==多进程COW`Copy On Write` 机制==来实现快照持久化
  ...
  >
  > ![](/Users/yinren/allText/learnNote/pics/redis/redis_10.png)
...
- **混合持久化**：将 RDB 文件的内容和增量的 AOF 日志文件放到一起
  ...
  >
  > ![](/Users/yinren/allText/learnNote/pics/redis/redis_11.png)
...
- **消息交互**：
  ...
  > **传统方式**：
  >
  > ![](/Users/yinren/allText/learnNote/pics/redis/redis_12.png)
  >
  > **改进**：
  >
  > ![](/Users/yinren/allText/learnNote/pics/redis/redis_13.png)
  >
  > ---
  >
  > 操作展示：
  ...
  > **传统操作**：
  >
  > ![](/Users/yinren/allText/learnNote/pics/redis/redis_14.png)
  >
  > **改进**：
  >
  > ![](/Users/yinren/allText/learnNote/pics/redis/redis_15.png)
  >
  > ---
  >
...
- **消息多播**：允许生产者只生产一次消息，由中间件负责将消息复制到多个消息队列，每个消息队列由相应的消费者进行消费
  ...
  > ![](/Users/yinren/allText/learnNote/pics/redis/redis_16.png)
  >
  > **消息队列下的情况**：
  >
  > ![](/Users/yinren/allText/learnNote/pics/redis/redis_17.png)
...
- `ziplist`：
  ...
  > ![](/Users/yinren/allText/learnNote/pics/redis/redis_18.png)
...
- **缓存击穿**：对于热点数据，当缓存失效的一瞬间，所有请求都被下放到数据库去请求更新缓存，数据库被压垮
  ...
  > 解决的办法：
  >
  > - **加全局锁**： 所有访问某个数据的请求都共享一个锁，获得锁的那个才有资格去访问数据库，其他线程必须等待，比如 Redis 的 setnx 实现全局锁
  > - **对即将过期的数据进行主动刷新**： 比如新起一个线程轮询数据，或者比如把所有的数据划分为不同的缓存区间，定期分区间刷新数据

- **缓存雪崩**：指当给所有的缓存设置了同样的过期时间，当某一时刻，整个缓存的数据全部过期，然后瞬间所有的请求都被抛向了数据库，数据库就崩掉了

  > 解决的办法：
  >
  > - **分治**： 划分更小的缓存区间，按区间过期
  > - **随机值**： 给每个key的过期时间加一个随机值，避免同时过期，达到错峰刷新缓存的目的
