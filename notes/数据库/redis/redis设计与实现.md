...
- 每两个结点会抽出一个结点作为上一级索引的结点，那第一级索引的结点个数大约就是 n/2，第二级索引的结点个数大约就是 n/4，第三级索引的结点个数大约就是 n/8，依次类推，第 k 级索引的结点个数是第 k-1 级索引的结点个数的 1/2，那第 k级索引结点的个数就是 $n/(2^k)$

- 假设索引有 h 级，最高级的索引有 2 个结点，可以得到 $n/(2^h)=2$，从而求得 $h=log2n-1$。如果包含原始链表这一层，整个跳表的高度就是 log2n
...
- 这几级索引的结点总和就是 n/2+n/4+n/8…+8+4+2=n-2，即跳表的空间复杂度是 O(n)
...
<img src="../../../pics/redis/redis_27.png" align=left width=600>

- **插入后的表**：

<img src="../../../pics/redis/redis_28.png" align=left width=600>
...
> 跳表通过随机函数来维护“平衡性”，即当往跳表中插入数据时，可以选择同时将这个数据插入到部分索引层

通过一个随机函数，来决定将这个结点插入到哪几级索引中，如：随机函数生成了值 K，则将这个结点添加到第一级到第 K 级索引中
...
- Redis 是内存数据库，Key 也存储在内存中，**过长的 Key 会增加 Redis 的整体内存占用**。
- 例如，一个 10 字节的 Key 比一个 100 字节的 Key 消耗的内存要少很多。

### **(2) 网络和 I/O 开销增大**

- Redis 主要通过 **TCP 协议** 进行通信，Key 过长会增加**网络传输的时间**，导致请求变慢。
- 在 **批量操作**（如 MGET、MSET）中，Key 过长会放大网络开销。

### **(3) 哈希查找性能下降**

- Redis 采用 **哈希表（Hash Table）** 存储 Key，过长的 Key 会影响哈希计算效率，**增加查找时间**。

### **(4) 影响 RDB/AOF 文件大小**

- Redis 持久化（RDB/AOF）会存储 Key，Key 过长会导致 **持久化文件增大**，影响磁盘 I/O 性能。

### **(5) 影响缓存命中率**

- 由于 Redis 是**基于内存的缓存系统**，Key 过长会导致 **更少的缓存数据被存储在内存中**，从而影响缓存命中率。

### **Redis Key 长度最佳实践**

| 场景              | 说明                                            |
| ----------------- | ----------------------------------------------- |
| **最大 Key 长度** | **推荐不超过 50 字节**，一般 **20-30 字节最优** |
| **Key 语义化**    | 尽量**短小且有意义**，避免冗余信息              |
| **使用缩写**      | 使用简洁的缩写，如 `usr` 代替 `user`            |
| **分隔符**        | `:` 作为分隔符，保证可读性                      |
| **Hash 结构**     | 用 Hash 存储多个属性，减少 Key 长度             |
