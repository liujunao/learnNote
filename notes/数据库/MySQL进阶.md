...
- Innodb存储引擎表是索引组织表，即**表中数据按主键顺序存放**，查询效率快
  ...
  > **优点**： 只要找到第一个索引值记录，其余就连续性的记录在物理也一样连续存放
  >
...
-  AHI 通过缓冲池的B+树构造而，且不需要对整张表构建哈希索引
...
- **一致性的非锁定读**： 指 InnoDB 存储引擎通过多版本控制的方式来读取当前执行时间数据库中行的数据
  ...
  > - 若读取的行正在执行 DELETE或 UPDATE操作，InnoDB 存储引擎会去读取行的一个**快照数据**
...
- `READ UNCOMMITTED`： **称为浏览访问，仅仅针对事务而言**，导致脏读
